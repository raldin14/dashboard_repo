...
export default function Home() {
  ...
  useEffect(() => {
    if (activeRunId) {
      const audit = data.audits.find((a) => a.runId === activeRunId);
      if (audit) setLiveLog(audit.log || []);
    }
  }, [data.audits, activeRunId]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      >
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  <span>{a.branch}</span>
                  {a.runId && (
                    <span title={`Full Execution ID: ${a.runId}`} style={{ color: '#aaa' }}>
                      • Run: {a.runId.split('-')[0]}
                    </span>
                  )}
                </div>
              ...
              <button
                ...
                style={{
                  background: a.status === 'pending' ? '#534AB7' : '#f0f0f0',
                  color: a.status === 'pending' ? 'white' : '#666',
                  padding: '4px 10px',
                  borderRadius: 6,
                  border: 'none',
                  cursor: 'pointer',
        ...
        >
          ...
          {(!audits[0] ||
            (!audits[0].finalReport && audits[0].status !== 'pending')) && (
            <div style={{ fontSize: 13, color: '#999', padding: '1rem 0' }}>
              No output yet
            </div>
          )}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <h2 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>
                  Execution Stream{' '}
                  {activeRunId &&
                    `(PR #${audits.find((a) => a.runId === activeRunId)?.prNumber})`}
                </h2>
              </div>
              <button
                      ...
                      {isLast && (
                        <div
                          ...
                          style={{
                            ...
                            width: 2,
                            background:
                              'linear-gradient(to bottom, #534AB7, transparent)',
                          }}
                        />
                      )}
                      <div
